...
test('hole: error when used inside lawof', () => {
  const errs = errors('m = lawof(_)\n');
  assert.ok(errs.some(d => /Hole.*fn/.test(d.message)));
});
...
test('placeholder: valid inside lawof', () => {
  assert.equal(errors('m = lawof(_x_, x = _x_)\n').length, 0);
});

test('placeholder: error when used outside reification', () => {
  const errs = errors('x = _par_ * 2\n');
  assert.ok(errs.some(d => /Placeholder.*functionof.*lawof/.test(d.message)));
});
